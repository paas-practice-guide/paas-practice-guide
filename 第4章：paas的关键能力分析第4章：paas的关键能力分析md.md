...
按照最佳实践，在运行时环境中运行的用户代码最好是无状态的（statless）。这样这些代码的高可用和可扩展就可以完全由PaaS平台本身来管理，降低开发者需要做出的努力。但除非是非常简单的Web类应用可以完全没有状态，比如1024之类的游戏，其他应用都会或多或少需要“状态”的维持来提升客户体验，那么这些状态又放在哪儿实现呢？十二因子给了我们一个答案，通过Backing Service来实现这些状态的托管。我们将其译作后端支持服务。
