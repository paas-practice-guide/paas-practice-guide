...
由于PaaS中托管的服务、后端服务实例、甚至PaaS管理平台本身都运行在IaaS平台上，另外PaaS云平台需要适配诸多私有环境。因此一个成功的PaaS云平台必须要具备一个合适的IaaS适配层。通过这个适配层屏蔽不同的IaaS的差别，这样就既可以部署在AWS、Azure等公有云之上，也可以部署在openstack和vsphere的私有环境中。
