...
* 制品库。除了Python和PHP等解释型语言，其他语言都需要经过一定形式的预编译或编译。比如Java需要编译成机器代码并加入其他代码包，Go语言需要编译成静态的机器代码等。如果使用Docker作为运行时环境的话，这些代码还需要加入基础镜像（Base Image）中，并打包成新的Docker Image。或者这些代码需要和其运行环境已经形成容器镜像等待运行。这些从代码生成的待运行的东西叫做制品（artfacts），等待运行时环境载入。对于Docker而言，其制品库就是镜像仓库。
