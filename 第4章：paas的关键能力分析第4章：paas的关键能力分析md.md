...
* 应用市场。PaaS平台中除了托管应用开发者自己开发的个性化应用之外，还需要能运行和定制化其他开发者开发的应用，这些应用就在应用市场中共其他人选用。它们或者是以代码的形式在其中运行，或者能提供一些Open API。应用市场有助于扩展PaaS云服务提供商自身的生态，不但能引入除了其自身提供的后端支持服务的其他外部组件，还可以帮助云平台中的开发者获取自己的收入。
...
通常PaaS平台会维护期望状态和实际状态的一致性，也就是说如果运行时环境出现非预期的问题，比如当机或程序异常中止，那么云平台会选择另外的节点继续运行此段代码，并通过技术机制使得最终用户和开发者都没有感知。
